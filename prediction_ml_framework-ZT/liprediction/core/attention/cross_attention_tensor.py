...
class BipartiteAttention(nn.Module, metaclass=ABCMeta):
    '''
    CrossAttention: Tensor version
    '''
    ...
    def Create(config):
        if 'type' in config:
            if config['type'] == 'DotProdMultiHead':
                return BipartiteAttentionDotProdMultiHead(config)
            else:
    ...
    @abstractmethod
    def forward(self, x_kv, x_q, x_kv_mask=None, x_q_mask=None):
        raise Exception('Not implemented')


class BipartiteAttentionDotProdMultiHead(BipartiteAttention):
    '''
    CrossAttention: Tensor version
    '''
    ...
    def __init__(self, config):
        ...
        if self.wo_reize:
            self.bypass_linear = nn.Linear(self.dst_size, self.out_size)        
        ...
        for i in range(self.head_num):
            self.lin_k.append(nn.Linear(self.src_size, self.key_size))
            self.lin_v.append(nn.Linear(self.src_size, self.val_size))
            self.lin_q.append(nn.Linear(self.dst_size, self.key_size))
    ...
    def forward(self, x_kv, x_q, x_kv_mask=None, x_q_mask=None):
        '''
        Args:
          x_kv: shape=[..., L_kv, F_kv]
          x_q: shape=[..., L_q, F_q]
          x_kv_mask: shape=[..., L_kv]
          x_q_mask: shape=[..., L_q]

        Return:
          out: [..., L_q, out_size]
          atten: [..., head_num, L_q, L_kv]
        '''

        head_values = []
        head_attens = []
        for i in range(self.head_num):
            # [..., L_kv, F_kv] -> [..., L_kv, key_size]
            k = self.lin_k[i](x_kv)
            # [..., L_kv, F_kv] -> [..., L_kv, val_size]
            v = self.lin_v[i](x_kv)
            # [..., L_q, F_q] -> [..., L_q, key_size]
            q = self.lin_q[i](x_q)
            ...
            if x_kv_mask is not None:
                ...
                if self.mask_wo_fill:
                    inverse_mask = (1.0 - x_kv_mask) * self.min_value
                    score = score + inverse_mask                
                else:
                    score = score.masked_fill((x_kv_mask == 0).unsqueeze(-2), self.min_value)
            ...
            # [head_num, [..., L_q, out_size]] or val_size
